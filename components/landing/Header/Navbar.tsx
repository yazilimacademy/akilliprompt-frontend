...
function Navbar() {
  ...
  return (
    <header className="sticky navbar-home top-0 p-5 w-full bg-transparent z-50 container mx-auto">
      <nav
        className="bg-background/70 backdrop-blur-lg border border-slate-400/40 max-w-4xl xl:max-w-5xl mx-auto px-0 p-1 lg:p-2 rounded-full">
        <div className="flex items-center justify-between px-5 lg:px-0">
              ...
              {menuItems.map((menuItem, index) => (
                <li key={index}>
                  <Link href={menuItem.url}>
                    <button
                      className="border-b border-transparent hover:border-b-foreground hover:text-foreground">{menuItem.text}</button>
                  </Link>
                </li>
              ))}
            </ul>
          ...
          <div className="flex items-center justify-center lg:hidden">
            <button
              className={cn('advanced-setting-toggle focus:outline-none transition-all duration-150',
                {
